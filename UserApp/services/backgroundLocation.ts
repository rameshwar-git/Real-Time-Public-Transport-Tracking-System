 ...
 *
 * Uses expo-location's background task (via expo-task-manager) so the app keeps reporting
 * its position while backgrounded / screen locked / removed from recents. The task posts via
 * the REST endpoint (a background task has no live Socket.IO connection). Works on dev/
 * release NATIVE builds — NOT in Expo Go.
 */
    ...
    try {
        ...
        await Location.startLocationUpdatesAsync(BACKGROUND_LOCATION_TASK, {
            ...
            foregroundService: {
                ...
                notificationBody: 'Sharing your location to track your ride in real time.',
